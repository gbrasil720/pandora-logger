...
export class Logger {
  ...
  constructor(filePath: string, encryptionOptions: EncryptionOptions) {
    ...
    if (
      ...
      (!encryptionOptions.encryptionKey ||
        encryptionOptions.encryptionKey.length !== 32)
    ) {
      throw new PandoraError(
  ...
  create(initialData: object) {
    ...
    const data = this.encryptionOptions.enabled
      ? this.encrypt(JSON.stringify(initialData))
  ...
  read(): any {
    ...
    const decryptedContent = this.encryptionOptions.enabled
  ...
  write(data: object) {
    ...
    const content = this.encryptionOptions.enabled
  ...
  filter(
    criteria: (key: string, message: string) => boolean
  ...
  private encrypt(text: string): string {
    if (!this.encryptionOptions.encryptionKey) {
      throw new PandoraError(
        'Encryption key not provided. Ensure the key is 32 characters for AES-256.',
        EMOJIS.WARNING ?? '',
        1002
      )
    }
  ...
  public decrypt(text: string): string {
    if (!this.encryptionOptions.encryptionKey) {
      throw new PandoraError(
        'Decryption key not provided. Ensure the key is 32 characters for AES-256.',
        EMOJIS.WARNING ?? '',
        1003
      )
    }
