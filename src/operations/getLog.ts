...
export async function getLog(client: PandoraClient, key: string): Promise<string | undefined> {
  await client.spinner.run(async () => {
    await delay(500);
  }, `Retrieving log for key: ${key}...`, `Log for key: ${key} founded and retrieved successfully!`);

  const logData = client.logger.read();
  const logMessage = logData.logs[key];

  if (logMessage) {
    console.log(`${clc.green(EMOJIS.CHECK_MARK)}Log retrieved: ${clc.yellow(`${key} -> ${logMessage}`)}`);
  } else {
